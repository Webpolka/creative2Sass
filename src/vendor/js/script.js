class Needle {
	constructor(elem, options) {
		const defaultConfig = {
			...
			videoText: "промокод в конце видео",
			...
			samolet: {
				...
				videoStorageLink: "https://storage.yandexcloud.net/creo/Samolet/samolet.mp4",
				videoStoragePoster: "./images/sber/sber-banner.png",
	...
	listener() {
		...
		this.thumb.addEventListener("touchend", (e) => {
			if (this.canMove && this.dragging && this.touchMoved) {
				...
				const video = this.creative.querySelector(".browser-player");
			
				const playPromise = video.play();
				if (playPromise !== undefined) {
					playPromise
						.then(() => {
							video.pause(); // Останавливаем сразу после старта (иначе Safari может блокировать позже)
							setTimeout(() => {
						...
						.catch((err) => {
							console.error("Нельзя начать воспроизведение:", err);
						});
				}
	...
	showPercentText(percent) {
		if (percent != 0) {
			...
			text.innerHTML = `<span class="${percentClass}">${percent}%</span> пользователей думают так же`;
		}
	}
	...
	openBrowserVideo() {
		this.videoContainer = this.creative.querySelector(".videoContainer");
		this.browserPlayer = this.creative.querySelector(".browser-player");
		this.videoCloseButton = this.creative.querySelector(".videoCloseButton");

		let creative = this.creative;
		let container = this.videoContainer;
	...
	createWidget() {
		let logoLink, landingLink, videoLink, videoPoster;
		...
		<video 
		class="videoPlayerClass browser-player" 
		playsinline		
		preload="auto"		
		poster="${videoPoster}" 
		controls
